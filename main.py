...
def create_ingredient_groups_html(ingredient_groups, color, color_dark):
    result = ''
    for ingredient_group in ingredient_groups:
        result += ''.join([f'<div class="ingredients-title" style="color:light-dark({color}, {color_dark})">{ingredient_group_title}</div>{create_ingredients_html(ingredient_group[ingredient_group_title])}' if ingredient_group[ingredient_group_title] is not None else '' for ingredient_group_title in ingredient_group])
    return result
...
def create_recipe_html(recipe):
    if 'dominant_color' in recipe:
        ...
        bg_color_dark = colorsys.hls_to_rgb(hls[0], 0.1, hls[2])
        bg_color_dark = [int(bg_color_dark[0] * 255), int(bg_color_dark[1] * 255), int(bg_color_dark[2] * 255)]
        bg_color_dark = color_hex(bg_color_dark)
        color = colorsys.hls_to_rgb(hls[0], 0.3, hls[2])
        color = [int(color[0] * 255), int(color[1] * 255), int(color[2] * 255)]
        color = color_hex(color)
        color_dark = colorsys.hls_to_rgb(hls[0], 0.7, hls[2])
        color_dark = [int(color_dark[0] * 255), int(color_dark[1] * 255), int(color_dark[2] * 255)]
        color_dark = color_hex(color_dark)
    else:
        bg_color = '#eeeeee'
    ...
    return f'''\
<div class="recipe">
  <div class="title" style="color:light-dark({color}, {color_dark})">{recipe["title"]}</div>
  <div class="ingredients" style="background-color:light-dark({bg_color}, {bg_color_dark})">
    {create_ingredient_groups_html(recipe["ingredient_groups"], color, color_dark)}
  </div>
  <div class="instructions">
    <div class="instructions-title" style="color:light-dark({color}, {color_dark})">Instructions</div>
